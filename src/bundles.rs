...
impl BundleConfig {
    pub fn create_bundles(
        &self,
        cpus: &[usize],
    ) -> Result<Vec<Box<dyn Bundle>>, Box<dyn Error>> {
        let mut bundles: Vec<Box<dyn Bundle>> = vec![];

        bundles.push(Box::new(TimeBundle::new(&[])?));

        if self.rapl    { bundles.push(Box::new(RaplBundle::new(&[])?));    }
        if self.cycles  { bundles.push(Box::new(CyclesBundle::new(cpus)?)); }
        if self.misses  { bundles.push(Box::new(MissesBundle::new(cpus)?)); }
        if self.cstates { bundles.push(Box::new(CStateBundle::new(cpus)?)); }
...
impl Bundle for CyclesBundle {
    ...
    fn read(&mut self) -> Result<HashMap<String, String>, Box<dyn Error>> {
        let total: u64 = self.counters.iter_mut()
            .map(|c| c.read())
...
impl MissesBundle {
    pub const COLUMNS: &'static [&'static str] = &["l1d_misses", "l1i_misses", "llc_misses", "branch_misses"];
}

const L1D_MISS: Cache = Cache { which: CacheId::L1D, operation: CacheOp::READ, result: CacheResult::MISS };
const L1I_MISS: Cache = Cache { which: CacheId::L1I, operation: CacheOp::READ, result: CacheResult::MISS };
const LLC_MISS: Cache = Cache { which: CacheId::LL,  operation: CacheOp::READ, result: CacheResult::MISS };
...
impl Bundle for MissesBundle {
    fn new(cpus: &[usize]) -> Result<Self, Box<dyn Error>> {
        let events = [MissEvent::L1d, MissEvent::L1i, MissEvent::Llc, MissEvent::Branch];
        let mut counters = HashMap::new();
    ...
    fn read(&mut self) -> Result<HashMap<String, String>, Box<dyn Error>> {
        ...
        for (name, cpu_counters) in &mut self.counters {
            let total: u64 = cpu_counters.iter_mut()
                .map(|c| c.read())
                .collect::<Result<Vec<_>, _>>()?
                .into_iter()
                .sum();
...
impl CStateBundle {
    pub const COLUMNS: &'static [&'static str] = &[
        "c1_core_residency", "c3_core_residency", "c6_core_residency", "c7_core_residency",
        "c2_pkg_residency",  "c3_pkg_residency",  "c6_pkg_residency",
        "c8_pkg_residency",  "c10_pkg_residency",
    ];
}

const CORE_EVENTS: &[&str] = &["c1-residency", "c3-residency", "c6-residency", "c7-residency"];
const PKG_EVENTS:  &[&str] = &["c2-residency", "c3-residency", "c6-residency", "c8-residency", "c10-residency"];
    ...
    fn new(cpus: &[usize]) -> Result<Self, Box<dyn Error>> {
        ...
        for &event_name in CORE_EVENTS {
            for &cpu in cpus {
                if let Some(counter) = build_dynamic_counter("cstate_core", event_name, cpu) {
                    core_counters.push(CStateCounter { counter, event_name });
                }
            }
        }
        ...
        for &event_name in PKG_EVENTS {
            if let Some(counter) = build_dynamic_counter("cstate_pkg", event_name, 0) {
                pkg_counters.push(CStateCounter { counter, event_name });
            }
        }

        Ok(Self { core_counters, pkg_counters })
    }

    fn enable(&mut self) -> Result<(), Box<dyn Error>> {
        for c in self.core_counters.iter_mut().chain(self.pkg_counters.iter_mut()) {
            c.counter.enable()?;
        }
        Ok(())
    }
